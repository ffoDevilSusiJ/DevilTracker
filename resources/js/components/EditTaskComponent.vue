  ...
  <div class="task-add">
    <div class="task-add__container">
      <div style="display: flex; justify-content: space-between; align-items:center;">
        <div class="task-add__title">{{edit ? 'Редактирование' : 'Создание'}} задачи</div>
        <span @click="$emit('close')" class="close">&times;</span>

      </div>
      <form id="form" class="add-project__form" :method="action" :action="route">
        <input type="hidden" name="_token" :value="csrf">
        <input type="hidden" name="project_id" :value="selectedProject.id">

        <div class="form-group">
          <div v-if="!edit" class="form-group__divider">
        ...
        <div class="form-group form-button">
          <input type="submit" name="signup" id="signup" class="form-submit" :value="edit ? 'Применить' : 'Создать'" />
        </div>
      </form>
